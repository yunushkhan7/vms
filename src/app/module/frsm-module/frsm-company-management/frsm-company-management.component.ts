...
@Component({
  selector: 'app-frsm-company-management',
  templateUrl: './frsm-company-management.component.html',
  styleUrls: ['./frsm-company-management.component.scss'],
})
export class FrsmCompanyManagementComponent implements OnInit {
  showLoader = false;
  companyManagementList: any;
  companyList: any = [];
  pagination: any = null;
      ...
      (response) => {
        ...
        if (response?.data) {
          ...
          if (totalpage.toString().includes('.')) {
            this.lastpage =
              Math.floor(response['recordCount'] / params.pageSize) + 1;
          } else {
            this.lastpage = Math.round(
              response['recordCount'] / params.pageSize
            );
          }
  ...
  sortData(name) {
    ...
    this.sortFieldName = name == 'floor' ? 'floor[0].Name' : name;
